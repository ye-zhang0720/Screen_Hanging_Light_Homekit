...
class WiFi_configParameter {
  public:
    /** 
        Create custom parameters that can be added to the WiFi_config setup web page
        @id is used for HTTP queries and must not contain spaces nor other special characters
    */
    WiFi_configParameter();
    WiFi_configParameter(const char *custom);
    WiFi_configParameter(const char *id, const char *label);
    WiFi_configParameter(const char *id, const char *label, const char *defaultValue, int length);
    WiFi_configParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom);
    WiFi_configParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom, int labelPlacement);
    ~WiFi_configParameter();
    // WiFi_configParameter& operator=(const WiFi_configParameter& rhs);
  ...
  private:
    WiFi_configParameter& operator=(const WiFi_configParameter&);
    const char *_id;
    const char *_label;
    char       *_value;
    int         _length;
...
{
  public:
    WiFi_config(Stream& consolePort);
    WiFi_config();
    ~WiFi_config();
    void WiFi_configInit();
    ...
    bool          addParameter(WiFi_configParameter *p);

    //returns the list of Parameters
    WiFi_configParameter** getParameters();
    ...
    void          setAPCallback( std::function<void(WiFi_config*)> func );
    ...
    // set the webapp title, default WiFi_config
    ...
    void          setHomekitRepairParameter(char * _SN, char * _Setup_ID, char * _Setup_code);
  ...
  private:
    ...
    String        _title                  = FPSTR(S_brand); // app title -  default WiFi_config
    ...
    int         _paramsCount          = 0;
    int         _max_params;
    WiFi_configParameter** _params    = NULL;
    ...
    std::function<void(WiFi_config*)> _apcallback;
