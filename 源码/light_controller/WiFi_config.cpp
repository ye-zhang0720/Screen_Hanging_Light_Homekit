 ...
 * WiFi_config.cpp
 * 
 * WiFi_config, a library for the ESP8266/Arduino platform
...
#ifdef ESP32
uint8_t WiFi_config::_lastconxresulttmp = WL_IDLE_STATUS;
#endif

/**
 * --------------------------------------------------------------------------------
 *  WiFi_configParameter
 * --------------------------------------------------------------------------------
**/

WiFi_configParameter::WiFi_configParameter() {
  WiFi_configParameter("");
}

WiFi_configParameter::WiFi_configParameter(const char *custom) {
  _id             = NULL;
  _label          = NULL;
  _length         = 1;
  _value          = NULL;
...
WiFi_configParameter::WiFi_configParameter(const char *id, const char *label) {
  init(id, label, "", 0, "", WFM_LABEL_DEFAULT);
}

WiFi_configParameter::WiFi_configParameter(const char *id, const char *label, const char *defaultValue, int length) {
  init(id, label, defaultValue, length, "", WFM_LABEL_DEFAULT);
}

WiFi_configParameter::WiFi_configParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom) {
  init(id, label, defaultValue, length, custom, WFM_LABEL_DEFAULT);
}

WiFi_configParameter::WiFi_configParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom, int labelPlacement) {
  init(id, label, defaultValue, length, custom, labelPlacement);
}

void WiFi_configParameter::init(const char *id, const char *label, const char *defaultValue, int length, const char *custom, int labelPlacement) {
  _id             = id;
  _label          = label;
...
WiFi_configParameter::~WiFi_configParameter() {
  ...
  _length=0; // setting length 0, ideally the entire parameter should be removed, or added to WiFi_config scope so it follows
}

// WiFi_configParameter& WiFi_configParameter::operator=(const WiFi_configParameter& rhs){
...
void WiFi_configParameter::setValue(const char *defaultValue, int length) {
  if(!_id){
...
const char* WiFi_configParameter::getValue() const {
  // Serial.println(printf("Address of _value is %p\n", (void *)_value)); 
  return _value;
}
const char* WiFi_configParameter::getID() const {
  return _id;
}
const char* WiFi_configParameter::getPlaceholder() const {
  return _label;
}
const char* WiFi_configParameter::getLabel() const {
  return _label;
}
int WiFi_configParameter::getValueLength() const {
  return _length;
}
int WiFi_configParameter::getLabelPlacement() const {
  return _labelPlacement;
}
const char* WiFi_configParameter::getCustomHTML() const {
  return _customHTML;
}



void WiFi_config::setHomekitRepairParameter(char * _SN, char * _Setup_ID, char * _Setup_code){
  SN = _SN;
  Setup_ID = _Setup_ID;
 ...
 * @access public
 * @param {[type]} WiFi_configParameter *p [description]
 */
bool WiFi_config::addParameter(WiFi_configParameter *p) {
  ...
  if(_params == NULL){
    #ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_DEV,F("allocating params bytes:"),_max_params * sizeof(WiFi_configParameter*));        
    #endif
    _params = (WiFi_configParameter**)malloc(_max_params * sizeof(WiFi_configParameter*));
  }
  ...
  if(_paramsCount == _max_params){
    ...
    DEBUG_WM(DEBUG_DEV,F("re-allocating params bytes:"),_max_params * sizeof(WiFi_configParameter*));    
    #endif
    WiFi_configParameter** new_params = (WiFi_configParameter**)realloc(_params, _max_params * sizeof(WiFi_configParameter*));
    #ifdef WM_DEBUG_LEVEL
 ...
 * @access public
 */
WiFi_configParameter** WiFi_config::getParameters() {
  return _params;
}
 ...
 * @access public
 */
int WiFi_config::getParametersCount() {
  return _paramsCount;
}
...
// constructors
WiFi_config::WiFi_config(Stream& consolePort):_debugPort(consolePort){
  WiFi_configInit();
}
...
void WiFi_config::WiFi_configInit(){
...
WiFi_config::~WiFi_config() {
  _end();
  // parameters
  // @todo below belongs to WiFi_configparameter
  if (_params != NULL){
    #ifdef WM_DEBUG_LEVEL
  ...
  // @todo remove event
  // WiFi.onEvent(std::bind(&WiFi_config::WiFiEvent,this,_1,_2));
  #ifdef ESP32
...
boolean WiFi_config::autoConnect() {
 ...
 */
boolean WiFi_config::autoConnect(char const *apName, char const *apPassword) {
  #ifdef WM_DEBUG_LEVEL
...
bool WiFi_config::setupHostname(bool restart){
  if(_hostname == "") {
    #ifdef WM_DEBUG_LEVEL
 ...
 * @return {[type]} [description]
 */
void WiFi_config::startWebPortal() {
 ...
 * @return {[type]} [description]
 */
void WiFi_config::stopWebPortal() {
  if(!configPortalActive && !webPortalActive) return;
  #ifdef WM_DEBUG_LEVEL
...
boolean WiFi_config::configPortalHasTimeout(){
...
void WiFi_config::setupHTTPServer(){
  ...
  server->on(G(R_root),       std::bind(&WiFi_config::handleRoot, this));
  server->on(G(R_wifi),       std::bind(&WiFi_config::handleWifi, this, true));
  server->on(G(R_wifinoscan), std::bind(&WiFi_config::handleWifi, this, false));
  server->on(G(R_wifisave),   std::bind(&WiFi_config::handleWifiSave, this));
  server->on(G(R_info),       std::bind(&WiFi_config::handleInfo, this));
  server->on(G(R_param),      std::bind(&WiFi_config::handleParam, this));
  server->on(G(R_paramsave),  std::bind(&WiFi_config::handleParamSave, this));
  server->on(G(R_restart),    std::bind(&WiFi_config::handleReset, this));
  server->on(G(R_exit),       std::bind(&WiFi_config::handleExit, this));
  server->on(G(R_close),      std::bind(&WiFi_config::handleClose, this));
  server->on(G(R_erase),      std::bind(&WiFi_config::handleErase, this, false));
  server->on(G(R_status),     std::bind(&WiFi_config::handleWiFiStatus, this));
  server->onNotFound (std::bind(&WiFi_config::handleNotFound, this));
  
  server->on(G(R_update), std::bind(&WiFi_config::handleUpdate, this));
  server->on(G(R_updatedone), HTTP_POST, std::bind(&WiFi_config::handleUpdateDone, this), std::bind(&WiFi_config::handleUpdating, this));
 ...
 * @return {[type]}      [description]
 */
boolean  WiFi_config::startConfigPortal(char const *apName, char const *apPassword) {
  _begin();
...
uint8_t WiFi_config::processConfigPortal(){
    if(configPortalActive){
      //DNS handler
 ...
 */
bool WiFi_config::shutdownConfigPortal(){
  #ifdef WM_DEBUG_LEVEL
...
uint8_t WiFi_config::connectWifi(String ssid, String pass, bool connect) {
  #ifdef WM_DEBUG_LEVEL
 ...
 */
bool WiFi_config::wifiConnectNew(String ssid, String pass,bool connect){
  bool ret = false;
  #ifdef WM_DEBUG_LEVEL
 ...
 * @return bool success
 */
bool WiFi_config::wifiConnectDefault(){
  bool ret = false;
...
void WiFi_config::updateConxResult(uint8_t status){
...
uint8_t WiFi_config::waitForConnectResult() {
  #ifdef WM_DEBUG_LEVEL
 ...
 */
uint8_t WiFi_config::waitForConnectResult(uint32_t timeout) {
  if (timeout == 0){
    #ifdef WM_DEBUG_LEVEL
...
String WiFi_config::getHTTPHead(String title){
  String page;
...
void WiFi_config::HTTPSend(String content){
 ...
 */
void WiFi_config::handleRequest() {
 ...
 */
void WiFi_config::handleWifi(boolean scan) {
  #ifdef WM_DEBUG_LEVEL
...
// bool WiFi_config::WiFi_scanNetworksForAP(bool force){
//   WiFi_scanNetworks(force);
// }

void WiFi_config::WiFi_scanComplete(int networksFound){
  _lastscan = millis();
...
bool WiFi_config::WiFi_scanNetworks(){
  return WiFi_scanNetworks(false,false);
}
 
bool WiFi_config::WiFi_scanNetworks(unsigned int cachetime,bool async){
    return WiFi_scanNetworks(millis()-_lastscan > cachetime,async);
}
bool WiFi_config::WiFi_scanNetworks(unsigned int cachetime){
    return WiFi_scanNetworks(millis()-_lastscan > cachetime,false);
}
bool WiFi_config::WiFi_scanNetworks(bool force,bool async){
    #ifdef WM_DEBUG_LEVEL
    ...
    if(force || (_lastscan>0 && (millis()-_lastscan > 60000))){
      ...
      if(async && _asyncScan){
          ...
          WiFi.scanNetworksAsync(std::bind(&WiFi_config::WiFi_scanComplete,this,_1));
          #else
...
String WiFi_config::WiFi_config::getScanItemOut(){
    String page;
...
String WiFi_config::getIpForm(String id, String title, String value){
...
String WiFi_config::getStaticOut(){
  String page;
...
String WiFi_config::getParamOut(){
  String page;
  ...
  if(_paramsCount > 0){
    ...
    for (int i = 0; i < _paramsCount; i++) {
      ...
      if (_params[i] == NULL || _params[i]->_length == 0 || _params[i]->_length > 99999) {
        ...
        #ifdef WM_DEBUG_LEVEL
        DEBUG_WM(DEBUG_ERROR,F("[ERROR] WiFi_configParameter is out of scope"));
        #endif
        return "";
      }
    }
...
void WiFi_config::handleWiFiStatus(){
  #ifdef WM_DEBUG_LEVEL
 ...
 */
void WiFi_config::handleWifiSave() {
  #ifdef WM_DEBUG_LEVEL
...
void WiFi_config::handleParamSave() {
...
void WiFi_config::handleInfo() {
  ...
  #endif
  page += F("<p1>Homekit配置信息</p1>");
  page += F("<dl>");
  page += F("<dt>序列号</dt><dd>");
  page += SN;
  page += F("<dt>配对码</dt><dd>");
  page += Setup_code;
  page += F("<dt>Setup ID</dt><dd>");
  page += Setup_ID;
  page += F("</dl>");
  page += F("<hr>");
...
String WiFi_config::getInfoData(String id){
...
// void WiFi_config::handleErase() {
//   handleErase(false);
// }
void WiFi_config::handleErase(boolean opt) {
  #ifdef WM_DEBUG_LEVEL
 ...
 * HTTPD CALLBACK 404
 */
void WiFi_config::handleNotFound() {
  if (captivePortal()) return; // If captive portal redirect instead of displaying the page
  handleRequest();
 ...
 */
boolean WiFi_config::captivePortal() {
  #ifdef WM_DEBUG_LEVEL
...
void WiFi_config::stopCaptivePortal(){
...
void WiFi_config::reportStatus(String &page){
 ...
 * @return {[type]} [description]
 */
bool WiFi_config::stopConfigPortal(){
  if(_configPortalIsBlocking){
    abort = true;
    return true;
  }
 ...
 * @access public
 */
bool WiFi_config::erase(){
  return erase(false);
}

bool WiFi_config::erase(bool opt){
  #ifdef WM_DEBUG_LEVEL
  DEBUG_WM("Erasing");
  #endif
 ...
 * @access public
 */
void WiFi_config::resetSettings() {
#ifdef WM_DEBUG_LEVEL
 ...
 */
void WiFi_config::setTimeout(unsigned long seconds) {
 ...
 * @param {[type]} unsigned long seconds [description]
 */
void WiFi_config::setConfigPortalTimeout(unsigned long seconds) {
 ...
 * @param {[type]} unsigned long seconds [description]
 */
void WiFi_config::setConnectTimeout(unsigned long seconds) {
 ...
 */
void WiFi_config::setConnectRetries(uint8_t numRetries){
 ...
 */
void WiFi_config::setCleanConnect(bool enable){
 ...
 * @param {[type]} unsigned long seconds [description]
 */
void WiFi_config::setSaveConnectTimeout(unsigned long seconds) {
 ...
 */
void WiFi_config::setSaveConnect(bool connect) {
 ...
 */
void WiFi_config::setDebugOutput(boolean debug) {
  _debug = debug;
  if(_debug && _debugLevel == DEBUG_DEV) debugPlatformInfo();
}

void WiFi_config::setDebugOutput(boolean debug, String prefix) {
  _debugPrefix = prefix;
 ...
 */
void WiFi_config::setAPStaticIPConfig(IPAddress ip, IPAddress gw, IPAddress sn) {
  _ap_static_ip = ip;
  _ap_static_gw = gw;
  _ap_static_sn = sn;
}
 ...
 * @param {[type]} IPAddress sn [description]
 */
void WiFi_config::setSTAStaticIPConfig(IPAddress ip, IPAddress gw, IPAddress sn) {
  _sta_static_ip = ip;
  _sta_static_gw = gw;
  _sta_static_sn = sn;
}
 ...
 */
void WiFi_config::setSTAStaticIPConfig(IPAddress ip, IPAddress gw, IPAddress sn, IPAddress dns) {
 ...
 */
void WiFi_config::setMinimumSignalQuality(int quality) {
 ...
 */
void WiFi_config::setBreakAfterConfig(boolean shouldBreak) {
 ...
 * @access public 
 * @param {[type]} void (*func)(WiFi_config* wminstance)
 */
void WiFi_config::setAPCallback( std::function<void(WiFi_config*)> func ) {
  _apcallback = func;
}
 ...
 */
void WiFi_config::setWebServerCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void WiFi_config::setSaveConfigCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void(*func)(void)
 */
void WiFi_config::setConfigResetCallback( std::function<void()> func ) {
    _resetcallback = func;
}
 ...
 * @param {[type]} void (*func)(void)
 */
void WiFi_config::setSaveParamsCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void WiFi_config::setPreSaveConfigCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void WiFi_config::setPreOtaUpdateCallback( std::function<void()> func ) {
 ...
 * @param char element
 */
void WiFi_config::setCustomHeadElement(const char* html) {
 ...
 * @param char element
 */
void WiFi_config::setCustomMenuHTML(const char* html) {
 ...
 */
void WiFi_config::setRemoveDuplicateAPs(boolean removeDuplicates) {
 ...
 */
void WiFi_config::setConfigPortalBlocking(boolean shouldBlock) {
 ...
 */
void WiFi_config::setRestorePersistent(boolean persistent) {
 ...
 */
void WiFi_config::setShowStaticFields(boolean alwaysShow){
 ...
 * @param boolean alwaysShow [false]
 */
void WiFi_config::setShowDnsFields(boolean alwaysShow){
 ...
 * @param boolean alwaysShow [false]
 */
void WiFi_config::setShowPassword(boolean show){
  _showPassword = show;
}
 ...
 */
void WiFi_config::setCaptivePortalEnable(boolean enabled){
 ...
 * @param boolean enabled [true]
 */
void WiFi_config::setWiFiAutoReconnect(boolean enabled){
 ...
 */
void WiFi_config::setAPClientCheck(boolean enabled){
 ...
 * @param boolean enabled [true]
 */
void WiFi_config::setWebPortalClientCheck(boolean enabled){
 ...
 * @param boolean enabled [false]
 */
void WiFi_config::setScanDispPerc(boolean enabled){
 ...
 * @param boolean enabled [true]
 */
void WiFi_config::setEnableConfigPortal(boolean enable)
{
 ...
 * @param boolean enabled [true]
 */
void WiFi_config::setDisableConfigPortal(boolean enable)
{
 ...
 */
bool  WiFi_config::setHostname(const char * hostname){
  //@todo max length 32
  _hostname = String(hostname);
  return true;
}

bool  WiFi_config::setHostname(String hostname){
  //@todo max length 32
  _hostname = hostname;
  return true;
}
 ...
 */
void WiFi_config::setWiFiAPChannel(int32_t channel){
  _apChannel = channel;
}
 ...
 */
void WiFi_config::setWiFiAPHidden(bool hidden){
  _apHidden = hidden;
}
 ...
 */
void WiFi_config::setShowInfoErase(boolean enabled){
 ...
 * @param boolean enabled
 */
void WiFi_config::setShowInfoUpdate(boolean enabled){
 ...
 */
bool WiFi_config::getConfigPortalActive(){
 ...
 * @return bool true if active
 */
bool WiFi_config::getWebPortalActive(){
  return webPortalActive;
}


String WiFi_config::getWiFiHostname(){
  #ifdef ESP32
 ...
 */
void WiFi_config::setTitle(String title){
  _title = title;
}
 ...
 *  WiFi_config.setMenu(menu);
 * @since $dev
 * @param uint8_t menu[] array of menu ids
 */
void WiFi_config::setMenu(const char * menu[], uint8_t size){
#ifdef WM_DEBUG_LEVEL
 ...
 */
void WiFi_config::setMenu(std::vector<const char *>& menu){
#ifdef WM_DEBUG_LEVEL
 ...
 * @since $dev
 */
void WiFi_config::setParamsPage(bool enable){
 ...
 */
String WiFi_config::getConfigPortalSSID() {
  return _apName;
}
 ...
 */
uint8_t WiFi_config::getLastConxResult(){
 ...
 */
bool WiFi_config::getWiFiIsSaved(){
 ...
 * @return string 
 */
String WiFi_config::getDefaultAPName(){
 ...
 */
void WiFi_config::setCountry(String cc){
  _wificountry = cc;
}
 ...
 */
void WiFi_config::setClass(String str){
  _bodyClass = str;
}
 ...
 */
void WiFi_config::setDarkMode(bool enable){
 ...
 */
void WiFi_config::setHttpPort(uint16_t port){
  _httpPort = port;
}


bool WiFi_config::preloadWiFi(String ssid, String pass){
  _defaultssid = ssid;
  _defaultpass = pass;
  return true;
}
 ...
 * @return String
 */
String WiFi_config::getWiFiSSID(bool persistent){
 ...
 * @return String
 */
String WiFi_config::getWiFiPass(bool persistent){
...
void WiFi_config::DEBUG_WM(Generic text) {
  DEBUG_WM(DEBUG_NOTIFY,text,"");
}

template <typename Generic>
void WiFi_config::DEBUG_WM(wm_debuglevel_t level,Generic text) {
  if(_debugLevel >= level) DEBUG_WM(level,text,"");
}

template <typename Generic, typename Genericb>
void WiFi_config::DEBUG_WM(Generic text,Genericb textb) {
  DEBUG_WM(DEBUG_NOTIFY,text,textb);
}

template <typename Generic, typename Genericb>
void WiFi_config::DEBUG_WM(wm_debuglevel_t level,Generic text,Genericb textb) {
 ...
 * @return {[type]} [description]
 */
void WiFi_config::debugSoftAPConfig(){
 ...
 * @return {[type]} [description]
 */
void WiFi_config::debugPlatformInfo(){
  #ifdef ESP8266
...
int WiFi_config::getRSSIasQuality(int RSSI) {
  int quality = 0;
...
/** Is this an IP? */
boolean WiFi_config::isIp(String str) {
...
/** IP to String? */
String WiFi_config::toStringIp(IPAddress ip) {
  String res = "";
  for (int i = 0; i < 3; i++) {
...
boolean WiFi_config::validApPassword(){
 ...
 */
String WiFi_config::htmlEntities(String str, bool whitespace) {
  str.replace("&","&amp;");
  str.replace("<","&lt;");
  str.replace(">","&gt;");
 ...
 * @return {[type]}         [description]
 */
String WiFi_config::getWLStatusString(uint8_t status){
  if(status <= 7) return WIFI_STA_STATUS[status];
  return FPSTR(S_NA);
}

String WiFi_config::getWLStatusString(){
  uint8_t status = WiFi.status();
  if(status <= 7) return WIFI_STA_STATUS[status];
  return FPSTR(S_NA);
}

String WiFi_config::encryptionTypeStr(uint8_t authmode) {
#ifdef WM_DEBUG_LEVEL
...
String WiFi_config::getModeString(uint8_t mode){
  if(mode <= 3) return WIFI_MODES[mode];
  return FPSTR(S_NA);
}

bool WiFi_config::WiFiSetCountry(){
...
bool WiFi_config::WiFi_Mode(WiFiMode_t m,bool persistent) {
    bool ret;
    #ifdef ESP8266
...
bool WiFi_config::WiFi_Mode(WiFiMode_t m) {
	return WiFi_Mode(m,false);
}

// sta disconnect without persistent
bool WiFi_config::WiFi_Disconnect() {
    #ifdef ESP8266
...
bool WiFi_config::WiFi_enableSTA(bool enable,bool persistent) {
#ifdef WM_DEBUG_LEVEL
...
bool WiFi_config::WiFi_enableSTA(bool enable) {
	return WiFi_enableSTA(enable,false);
}

bool WiFi_config::WiFi_eraseConfig() {
    #ifdef WM_DEBUG_LEVEL
...
uint8_t WiFi_config::WiFi_softap_num_stations(){
  #ifdef ESP8266
...
bool WiFi_config::WiFi_hasAutoConnect(){
  return WiFi_SSID(true) != "";
}

String WiFi_config::WiFi_SSID(bool persistent) const{
...
String WiFi_config::WiFi_psk(bool persistent) const {
    #ifdef ESP8266
    struct station_config conf;
  ...
  void WiFi_config::WiFiEvent(WiFiEvent_t event,arduino_event_info_t info){
  #else
  void WiFi_config::WiFiEvent(WiFiEvent_t event,system_event_info_t info){
...
void WiFi_config::WiFi_autoReconnect(){
  #ifdef ESP8266
    ...
    // if(_wifiAutoReconnect){
      ...
      if(wm_event_id == 0) wm_event_id = WiFi.onEvent(std::bind(&WiFi_config::WiFiEvent,this,_1,_2));
    // }
  #endif
}

// Called when /update is requested
void WiFi_config::handleUpdate() {
  #ifdef WM_DEBUG_LEVEL
...
// upload via /u POST
void WiFi_config::handleUpdating(){
  // @todo
...
void WiFi_config::handleUpdateDone() {
